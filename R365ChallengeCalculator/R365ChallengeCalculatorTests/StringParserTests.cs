...
namespace R365ChallengeCalculatorTests
{
    ...
    public class StringParserTests
    {
        ...
        [DataRow(new string[] { "44", "3", "3" }, "//[^]44,3^3")]
        [DataRow(new string[] { "4", "2", "3" }, "//[-]4-2-3")]
        [DataRow(new string[] { "11", "22", "33" }, "//[***]\n11***22***33")]
        public void Parse_MultiCharacterCustomDelimetersShouldSucceed(string[] expected, string input)
        {
            var parser = new StringParser();
            var result = parser.Parse(input);
            CollectionAssert.AreEqual(expected, result);
        ...
        [DataTestMethod]
        [DataRow(new string[] { "4", "3", "3" }, "//[#][&&]4#3&&3")]
        [DataRow(new string[] { "11", "22", "hh", "33", "44" }, "//[*][!!][r9r]\n11r9r22*hh*33!!44")]
        public void Parse_MultipleMultiCharacterCustomDelimetersShouldSucceed(string[] expected, string input)
        {
            var parser = new StringParser();
            var result = parser.Parse(input);
            CollectionAssert.AreEqual(expected, result);
